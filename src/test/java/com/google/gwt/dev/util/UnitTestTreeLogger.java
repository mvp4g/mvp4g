 ...
 */
public class UnitTestTreeLogger
  extends TreeLogger {
  ...
  public void assertCorrectLogEntries() {
    ...
    for (int i = 0, c = expectedEntries.size(); i < c; ++i) {
      assertCorrectLogEntry(expectedEntries.get(i),
                            actualEntries.get(i));
    }
  }

  private static void assertCorrectLogEntry(LogEntry expected,
                                            LogEntry actual) {
    Assert.assertEquals("Log types do not match",
                        expected.getType(),
                        actual.getType());
    ...
    if (expected.getCaught() == null) {
      ...
      Assert.assertNotNull("Actual log exception type should not have been null",
                           actual.getCaught());
      Assert.assertTrue("Actual log exception type (" +
                        ...
                        ")",
                        expected.getCaught()
                                .isAssignableFrom(actual.getCaught()));
    }
  }
  ...
  @Override
  public TreeLogger branch(Type type,
                           String msg,
                           Throwable caught,
                           HelpInfo helpInfo) {
    log(type,
  ...
  public void log(Type type,
    ...
    LogEntry actualEntry = new LogEntry(type,
                                        msg,
                                        caught);
  ...
  public static class Builder {
    ...
    public void expectInfo(String msg,
                           Class<? extends Throwable> caught) {
      expect(TreeLogger.INFO,
             msg,
             caught);
    }

    public void expectSpam(String msg,
                           Class<? extends Throwable> caught) {
      expect(TreeLogger.SPAM,
             msg,
             caught);
    }
