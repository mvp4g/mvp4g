...
//    presenter.onSelectUser(user);
    ...
    assertEquals(rolesChoices.getSelectedIndex(),
                 0);
...
//    presenter.onSelectUser(user);
    ...
    view.getSelectedRolesListBox()
        .setSelectedIndex(1);

    MyMockListBox selectedRoles = (MyMockListBox) view.getSelectedRolesListBox();

    assertEquals(2,
                 selectedRoles.getItems()
                              .size());

    MyMockButton remove = (MyMockButton) view.getRemoveButton();
    remove.setEnabled(true);
    remove.getClickHandler()
          .onClick(MockGwtEvent.CLICK_EVENT);
...
//    presenter.onSelectUser(user);
    ...
    assertEquals(rolesChoices.getSelectedValue(),
                 selectedRoles.getItems()
                              .get(0));
    assertEquals(rolesChoices.getSelectedValue(),
                 user.getRoles()
                     .get(0));
    assertEquals(view.getError(),
                 null);
  ...
  public void testAddEnabled() {
    ...
    rolesChoices.getClickHandler()
                .onClick(MockGwtEvent.CLICK_EVENT);
    assertFalse(add.isEnabled());

    rolesChoices.getKeyUpHandler()
...
//    presenter.onSelectUser(createUserBean(new String[]{"test"}));
    selectedRoles.getClickHandler()
                 .onClick(MockGwtEvent.CLICK_EVENT);
    assertFalse(remove.isEnabled());
    selectedRoles.getKeyUpHandler()
                 .onKeyUp(MockGwtEvent.KEY_UP_EVENT);
    assertFalse(remove.isEnabled());
...
//    presenter.onSelectUser(createUserBean(new String[]{"test"}));
  ...
  private void assertDisable() {
    assertFalse(((MyMockButton) view.getRemoveButton()).isEnabled());
    assertFalse(((MyMockButton) view.getAddButton()).isEnabled());
    assertEquals(((MyMockListBox) view.getSelectedRolesListBox()).getItems()
                                                                 .size(),
                 0);
    MyMockListBox rolesChoices = (MyMockListBox) view.getRoleChoiceListBox();
    assertEquals(rolesChoices.getSelectedIndex(),
