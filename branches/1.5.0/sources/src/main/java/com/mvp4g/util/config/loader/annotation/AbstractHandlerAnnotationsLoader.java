...
public abstract class AbstractHandlerAnnotationsLoader<A extends Annotation, T extends EventHandlerElement> extends Mvp4gAnnotationsWithServiceLoader<A> {
	 ...
	 */
	@Override
	Mvp4gWithServicesElement loadElementWithServices( JClassType c, A annotation, Mvp4gConfiguration configuration ) throws Mvp4gAnnotationException {

		String className = c.getQualifiedSourceName();

		String eventHandlerName = buildElementNameIfNeeded( getAnnotationName( annotation ), className, "" );

		T eventHandler = loadHandler( c, annotation, configuration );
		...
		addElement( getConfigList( configuration ), eventHandler, c, null );

		return eventHandler;
	}

	abstract protected T loadHandler( JClassType c, A annotation, Mvp4gConfiguration configuration ) throws Mvp4gAnnotationException;
