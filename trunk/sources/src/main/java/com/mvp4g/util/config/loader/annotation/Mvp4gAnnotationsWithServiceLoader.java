...
/**
 * Class responsible for loading information contained in annotations from
 * classes where methods can contain <code>InjectService</code> annotation.
 * 
 * @author plcoirier
 * 
 */
public abstract class Mvp4gAnnotationsWithServiceLoader<T extends Annotation>
		extends Mvp4gAnnotationsLoader<T> {
	 ...
	 */
	@Override
	protected void loadElement(JClassType c, T annotation,
			Mvp4gConfiguration configuration) throws Mvp4gAnnotationException {
		Mvp4gWithServicesElement element = loadElementWithServices(c,
			...
			if (serviceAnnotation != null) {
				if (!m.isPublic()) {
					...
					throw new Mvp4gAnnotationException(c
							.getQualifiedSourceName(), m.getName(), err);
				}
				...
				element.getInjectedServices().add(
						new InjectedElement(serviceName, m.getName()));
			}
		}
	...
	/**
	 * Retrieve the name of service element with the given service class name.
	 * If no service with the given class name is found, create one.
	 * 
	 * @param configuration
	...
	private String getServiceName(Mvp4gConfiguration configuration,
		...
		if (serviceName == null) {
			...
			try {
				...
				service.setClassName(generatedClassName.substring(0,
	...
	abstract Mvp4gWithServicesElement loadElementWithServices(JClassType c,
			T annotation, Mvp4gConfiguration configuration)
			throws Mvp4gAnnotationException;
